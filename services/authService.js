...
exports.loginService = async ({ email, password, ip }) => {
    ...
    if (!isMatch) {
        ...
        if (attempts15 >= 10) {
            ...
            // 🔥 Escalation (3 locks → permanent block)
            if (user.lockCount >= 3) {
                user.isBlocked = true;
            }
        ...
        if (attempts5 >= 5) {
            const delay = Math.pow(2, user.lockCount) * 30000; // 30s, 1m, 2m...

            throw { code: 429, message: `Too many attempts. Wait ${delay / 1000}s` };
        }
